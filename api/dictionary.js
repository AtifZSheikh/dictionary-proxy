...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ----- Oxford (dictionaryapi.dev) -----
    try {
      const ox = await fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${word}`);
      if (ox.ok) {
        const oxJson = await ox.json();
        oxfordDef =
          oxJson[0]?.meanings?.[0]?.definitions?.[0]?.definition || oxfordDef;
      }
    } catch {}

    // ----- Collins scraping fallback -----
    try {
      const collinsPage = await fetch(
        `https://www.collinsdictionary.com/dictionary/english/${word}`,
        { headers: { "User-Agent": "Mozilla/5.0" } }
      ).then(r => r.text());
      const $c = cheerio.load(collinsPage);
      collinsDef =
        $c(".def").first().text().trim() ||
        $c(".definition").first().text().trim() ||
        collinsDef;
    } catch {}

    // ----- Thesaurus synonyms scraping -----
    try {
      const thesPage = await fetch(
        `https://www.thesaurus.com/browse/${word}`,
        { headers: { "User-Agent": "Mozilla/5.0" } }
      ).then(r => r.text());
      const $t = cheerio.load(thesPage);
      synonyms = $t("[data-testid='synonym-link']")
        .map((i, el) => $t(el).text().trim())
        .get();
    ...
    const html = `
      ...
      <body>
        <h1>Word: ${word}</h1>
        <h2>Oxford Definition</h2>
        <p>${oxfordDef}</p>
        <h2>Collins Definition</h2>
        <p>${collinsDef}</p>
